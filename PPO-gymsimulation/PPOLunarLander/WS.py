...
class PPOAgent:
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            # Wasserstein的：
            wasserstein_distance = torch.mean(torch.abs(torch.exp(old_logprobs) - torch.exp(logprobs)))

            # 算Surrogate Loss
            surrogate_loss = -(torch.exp(logprobs - old_logprobs.detach()) * wasserstein_distance)
            loss = surrogate_loss + 0.5 * self.MseLoss(state_values, rewards) - 0.01 * dist_entropy + 0.1 * wasserstein_distance
...
np.savetxt('LunarLander-WS.csv',real_rewards , delimiter = ',')
