...
class PolicyNet(nn.Module):#离散的概率分布
...
class PPO:
    def __init__(self, n_states, n_hiddens, n_actions,
                 actor_lr, critic_lr, lmbda, epochs, gamma, alpha, device):
        # 实例化策略网络
        ...
        self.alpha = alpha  # PPO-alpha散度中的alpha参数
        self.device = device
    ...
    def learn(self, transition_dict):
        ...
        for _ in range(self.epochs):
            ...
            # 计算alpha散度
            alpha_divergence = torch.sum(torch.exp(old_log_probs - log_probs) - 1)
            # alpha散度版本的PPO损失函数
            actor_loss = -torch.mean(alpha_divergence * advantage)
...
agent = PPO(n_states=n_states,  # 状态数
            ...
            epochs=10,  # 一组序列训练的轮次
            gamma=gamma,  # 折扣因子
            alpha=alpha,  # PPO-alpha散度中的alpha参数
            device=device
            )
real_array = []
# 训练
for j in range (whole_episodes):
    ...
    real_array.append(return_list)
    print(real_array)
np.savetxt('cartpole-Hellinge2222r.csv', real_array, delimiter = ',')
print(real_array)
