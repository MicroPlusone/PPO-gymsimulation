...
class PPO:
    def __init__(self, n_states, n_hiddens, n_actions,
                 actor_lr, critic_lr, lmbda, epochs, gamma, device):
        # 实例化策略网络
    ...
    def learn(self, transition_dict):
        ...
        for _ in range(self.epochs):
            ...
            # 计算KL散度
            kl_divergence = torch.sum(old_log_probs - log_probs)
            # KL散度版本的PPO损失函数
            actor_loss = -torch.mean(kl_divergence * advantage)
...
for j in range(whole_episodes):
    ...
    print(real_array)
# real_list = np.sum(real_array, axis=0)
np.savetxt('cartpole-KL.csv', real_array, delimiter = ',')
print(real_array)
# plt.plot(real_array)
# plt.title('return')
# plt.show()