    ...
    def update(self):
        ...
        for _ in range(self.ppo_epoch):
            for index in BatchSampler(
                ...
                wasserstein_distance = torch.mean(action_log_prob - old_action_log_prob[index])

                L1 = ratio * advantage[index]
                L2 = torch.clamp(ratio, 1 - self.clip_param, 1 + self.clip_param) * advantage[index]
                actor_loss = -torch.min(L1, L2).mean() - (torch.exp(action_log_prob - old_action_log_prob[index].detach()) * wasserstein_distance)
...
def main():
    ...
    for j in range(10):
        ...
        #     # agent.save_param()
        #     break
    np.savetxt('pendulum-WS.csv', final, delimiter=',')
    print(final)
