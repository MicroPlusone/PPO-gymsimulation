    ...
    def update(self):
        ...
        for _ in range(self.ppo_epoch):
            for index in BatchSampler(
                ...
                kl_divergence = F.kl_div(action_log_prob, old_action_log_prob[index], reduction='batchmean')

                L1 = ratio * advantage[index]
                L2 = torch.clamp(ratio, 1 - self.clip_param, 1 + self.clip_param) * advantage[index]
                actor_loss = -torch.min(L1, L2).mean() + self.kl_coeff * kl_divergence  # Add KL penalty term
    ...
    for j in range(10):
        for i_epoch in range(1000000): # 1000
            ...
            if i_epoch % 10 == 0:
                ...
                #     # agent.save_param()
                #     break
        np.savetxt('pendulum-KL.csv', final, delimiter=',')
        print(final)
        # if running_reward > -200:
        #     print("Solved! Moving average score is now {}!".format(running_reward))
        #     env.close()
