...
class PPO:
    ...
    def take_action(self, state):
        ...
        #print(action)
        #action = np.random.choice(np.arange(self.n_actions), p=probs.cpu().detach().numpy()[0])
        return action
...
n_actions = env.action_space.n  # 动作数 3 前 后 不推
#n_actions = 3
# 模型构建
...
for j in range(whole_episodes):
    ...
    print(real_array)
np.savetxt('mountaincar-Clip.csv', real_array, delimiter=',')
print(real_array)
