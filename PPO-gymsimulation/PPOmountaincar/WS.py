...
class PPO:
    ...
    def learn(self, transition_dict):
        ...
        for _ in range(self.epochs):
            ...
            # 计算WS距离
            wasserstein_distance = torch.mean(torch.abs(torch.exp(old_log_probs) - torch.exp(log_probs)))
            # 损失函数
            actor_loss = -torch.mean(wasserstein_distance * advantage)
...
for j in range(whole_episodes):
    ...
    print(real_array)
np.savetxt('mountaincar-WS.csv', real_array, delimiter = ',')
print(real_array)
